...
def MinMax(couleur, profondeur):
    ...
    if P and (P1 or (not P1 and not D1)):
        for pion in P:
            ...
            if manger:
                ScoresFinaux.append([max(scores), [dispo[0][scores.index(
                    max(scores))], dispo[1][scores.index(max(scores))]], pion])
            else:
                ScoresFinaux.append(
                    [max(scores), [dispo[0][scores.index(max(scores))], dispo[1]], pion])
    ...
    if D and (D1 or (not D1 and not P1)):
        for pion in D:
            ...
            if manger:
                ScoresFinaux2.append([max(scores), [dispo[0][scores.index(
                    max(scores))], dispo[1][scores.index(max(scores))]], pion])
            else:
                ScoresFinaux2.append(
                    [max(scores), [dispo[0][scores.index(max(scores))], dispo[1]], pion])
        ...
        for pion in P:
            ...
            for i in range(len(coups)):
                ...
                if manger:
                    ...
                    Annule(pion, coups[i], False, manger, couleur, mem)
            if manger:
                ScoresFinaux.append([min(scores), [dispo[0][scores.index(
                    min(scores))], dispo[1][scores.index(min(scores))]], pion])
            else:
                ScoresFinaux.append(
                    [min(scores), [dispo[0][scores.index(min(scores))], dispo[1]], pion])
        ...
        for pion in D:
            ...
            for i in range(len(coups)):
                ...
                if manger:
                    ...
                    Annule(pion, coups[i], True, manger, couleur, mem)
            if manger:
                ScoresFinaux2.append([min(scores), [dispo[0][scores.index(
                    min(scores))], dispo[1][scores.index(min(scores))]], pion])
            else:
                ScoresFinaux2.append(
                    [min(scores), [dispo[0][scores.index(min(scores))], dispo[1]], pion])
...
def Affiche(PartieGagnee=False):
    ...
    for i in range(0, WIDTH, int(WIDTH/5)):
        for j in range(0, WIDTH, int(WIDTH/5)):
            canvas.create_rectangle(
                i, j, i+int(WIDTH/10), j+int(WIDTH/10), fill="#C8AD7F")
            canvas.create_rectangle(
                i+int(WIDTH/10), j+int(WIDTH/10), i+int(WIDTH/5), j+int(WIDTH/5), fill="#C8AD7F")

    canvas.create_rectangle(0, WIDTH, WIDTH, WIDTH+int(WIDTH/10), fill="white")

    for i in range(11):
        for j in range(11):
            canvas.create_line(j*int(WIDTH/10), 0, j *
                               int(WIDTH/10), WIDTH, fill="black", width="1")
            canvas.create_line(0, i*int(WIDTH/10), WIDTH,
                               i*int(WIDTH/10), fill="black", width="1")
    ...
    for x in range(10):
        for y in range(10):
            ...
            yc = y * int(WIDTH/10)
            if (Grille[x][y] == 1):
                canvas.create_oval(xc+int(WIDTH/100), yc+int(WIDTH/100), xc+int(
                    WIDTH*(9/100)), yc+int(WIDTH*(9/100)), fill="#FEFEE2", width=str(WIDTH/200))
            if (Grille[x][y] == 2):
                canvas.create_oval(xc+int(WIDTH/100), yc+int(WIDTH/100), xc+int(
                    WIDTH*(9/100)), yc+int(WIDTH*(9/100)), fill="#2F1E0E", width=str(WIDTH/200))
            if (Grille[x][y] == 3):
                canvas.create_oval(xc+int(WIDTH/100), yc+int(WIDTH/100), xc+int(
                    WIDTH*(9/100)), yc+int(WIDTH*(9/100)), fill="#FEFEE2", width=str(WIDTH/200))
                canvas.create_text(xc+int(WIDTH/20), yc+int(WIDTH/20),
                                   font=('Arial', int(WIDTH*(4/100))), text="Q", fill='black')
            if (Grille[x][y] == 4):
                canvas.create_oval(xc+int(WIDTH/100), yc+int(WIDTH/100), xc+int(
                    WIDTH*(9/100)), yc+int(WIDTH*(9/100)), fill="#2F1E0E", width=str(WIDTH/200))
                canvas.create_text(xc+int(WIDTH/20), yc+int(WIDTH/20),
                                   font=('Arial', int(WIDTH*(4/100))), text="Q", fill='white')

    msg = 'SCORES : ' + str(Scores[0]) + '-' + str(Scores[1])
    fillcoul = 'black'
    canvas.create_text(int(WIDTH/2), WIDTH+int(WIDTH*(6/100)),
                       font=('Mistral', int(WIDTH*(9/100))), text=msg, fill=fillcoul)
...
def PionJoue(event, x, y, Dame):
    ...
    # tester si la position pour ce pion est valide
